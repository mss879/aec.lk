...
export function UniversitiesCollegesContent() {
  const [searchQuery, setSearchQuery] = useState("");
  const [activeFilter, setActiveFilter] = useState("all"); // all, universities, colleges

  // Handle hash anchor routing on load
  // #universities / #pathway select a tab and scroll to the directory. This has
  // to react to hash changes too: arriving from the nav while already on this
  // page is a same-document navigation, so the effect would never re-run.
  useEffect(() => {
    const applyHash = () => {
      const hash = window.location.hash;
      if (hash !== "#pathway" && hash !== "#universities") return;
      setActiveFilter(hash === "#pathway" ? "colleges" : "universities");
